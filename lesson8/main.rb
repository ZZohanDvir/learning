...
class Railway 
  ...
  def show_cars
    ...
    @trains.each { |train| train_choice = train if train.number == train_number }
    if train_choice.type == :cargo
      train_choice.each_car { |car| puts "Вагон с номером #{car.number}. Общий объем - #{car.total_capacity}, 
      свободный объем - #{car.available_capacity}, занятый объем - #{car.unavailable_capacity}" }
    elsif train_choice.type == :passenger
      train_choice.each_car { |car| puts "Вагон с номером #{car.number}. Общее количество мест - #{car.total_capacity}, 
